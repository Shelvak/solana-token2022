...
describe("transfer-hook", () => {
  ...
  it("testeando authority", async function () {

    let original = await provider.connection.getTokenAccountBalance(authorityATA)
    ...
    expect(Number((
      await provider.connection.getTokenAccountBalance(authorityATA)
    ).value.amount)).to.be.equal(Number(original.value.amount) + 1)

    // set new authority
    const newAuthority = await newAccountWithLamports(provider.connection);
    ...
    //   ASSOCIATED_TOKEN_PROGRAM_ID
    // );
    let multisig = await createMultisig(
      provider.connection,
      authority,
    ...
    console.log('Authority set', multisig.toBase58())
    ...
    // expect(Number((
    //   await provider.connection.getTokenAccountBalance(authorityATA)
    // ).value.amount)).to.be.equal(Number(original.value.amount) + 2)
    await sendAndConfirmTransaction(
        provider.connection,
        new Transaction().add(
          createMintToInstruction(
    ...
    // expect(Number((
    //   await provider.connection.getTokenAccountBalance(authorityATA)
    // ).value.amount)).to.be.equal(Number(original.value.amount) + 3)
    await sendAndConfirmTransaction(
        provider.connection,
        new Transaction().add(
          createMintToInstruction(
  ...
  it.skip("wait for interest - Local validator doesn't support interest (?)", async function () {
    const res = await sendAndConfirmTransaction(
      provider.connection,
      new Transaction().add(
        createMintToInstruction(
    ...
    console.log("Arranque: ", await provider.connection.getTokenAccountBalance(authorityATA))
    for (let i = 0; i < 120; i++) {
      ...
      let status = await provider.connection.confirmTransaction(signature)
      console.log("Con ", i * 50, " : ", status)


      console.log(`Con ${i * 50} : `, await provider.connection.getTokenAccountBalance(authorityATA))
